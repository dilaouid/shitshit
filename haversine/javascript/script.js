...
* @param {object} start The starting point position
* @param {object} destination The destination point position
* @return {number}  return the distance in meters between the two points, or -1 if an error occured.
*/
const haversine = (start, destination) => {
    var errors = [];
    try {
        [start, destination].map( (el, i) => {
